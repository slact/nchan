...
#### Download Packages
 ...
 - **Ubuntu / Debian (GetPageSpeed)**: Pre-built dynamic module packages are freely available from the GetPageSpeed repository for Debian 12/13 and Ubuntu 20.04/22.04/24.04 (amd64 and arm64). See [setup instructions](https://apt-nginx-extras.getpagespeed.com/apt-setup/), then install with `sudo apt-get install nginx nginx-module-nchan`. The module is automatically enabled after installation.
