...
static ngx_int_t memstore_slots[NGX_MAX_PROCESSES];
static ngx_int_t memstore_slots_count;
...
#else
  ngx_int_t       i, slot;
  i = h % memstore_slots_count;
  assert(i >= 0);
  slot = memstore_slots[i];

  if(slot == NCHAN_INVALID_SLOT) {
    ERR("something went wrong, the channel owner is invalid. i: %i h: %ui, workers: %i", i, h, memstore_slots_count);
    assert(0);
    return NCHAN_INVALID_SLOT;
  }
  
  //ERR("owner of %V is slot %i. own slot is %i", str, slot, ngx_process_slot);
  return slot;
#endif
}

ngx_int_t memstore_channel_owner(ngx_str_t *id) {
  ngx_int_t self = memstore_slot();
  ngx_int_t str_owner = memstore_str_owner(id);
  return nchan_channel_id_is_multi(id) ? self : str_owner;
}
...
static ngx_int_t nchan_store_init_worker(ngx_cycle_t *cycle) {
  ...
  for(i = 0; i < NGX_MAX_PROCESSES; i++) {
    if(memstore_slots[i] == ngx_process_slot) {
...
static ngx_int_t nchan_store_init_module(ngx_cycle_t *cycle) {
  ...
  ipc_set_alert_handler(ipc, memstore_ipc_alert_handler);
  
  tmpslots = ipc_get_worker_slots(ipc, &n);
  memstore_slots_count = n;
  ERR("n = %i", n);
  ERR("memstore_slots_count = %i", memstore_slots_count);
  for(i=0; i<NGX_MAX_PROCESSES; i++) {
    memstore_slots[i] = i < n ? tmpslots[i] : NCHAN_INVALID_SLOT;
    if(i<n) {
      ERR("slots[%i]=%i", i, memstore_slots[i]);
    }
  }
...
static void nchan_store_exit_worker(ngx_cycle_t *cycle) {
  ...
  for(i = 0; i < NGX_MAX_PROCESSES; i++) {
    if(ngx_process_slot == memstore_slots[i]) {
      my_procslot_index = i;
      break;
    }
  }
